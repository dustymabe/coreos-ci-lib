...
def call(params = [:]) {
    ...
    kolaRuns["run"] = {
        ...
        Ids += id
        shwrap("cd ${cosaDir} && cosa kola run ${rerun} --output-dir=${outputDir}/${id} --denylist-test basic --build=${buildID} ${arch} ${platformArgs} --tag '!reprovision' --parallel ${parallel} ${args} ${extraArgs}")
