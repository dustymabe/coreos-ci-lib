...
def call(params = [:]) {
    ...
    if (!params['skipMetal4k']) {
        ...
        if (arch != 's390x') {
            testIsoRuns1["metal4k"] = {
                ...
                Ids += id
                def scenariosArg = scenarios4k == "" ? "" : "--scenarios ${scenarios}"
                scenariosArg = scenarios4k == "" ? "" : "--scenarios ${scenarios}"
    ...
    if (!params['skipUEFI']) {
        // only aarch64 and x86_64 support UEFI
        if (arch in ['aarch64', 'x86_64']) {
            testIsoRuns2["metalUEFI"] = {
                def id = marker == "" ? "kola-testiso-uefi" : "kola-testiso-uefi-${marker}"
                Ids += id
                shwrap("cosa shell -- mkdir -p tmp/${id}")
                shwrap("cosa kola testiso -S --qemu-firmware=uefi ${extraArgsUEFI} --scenarios ${scenariosUEFI} --output-dir tmp/${id}/insecure")
                shwrap("cosa kola testiso -S --qemu-firmware=uefi-secure ${extraArgsUEFI} --scenarios ${scenariosUEFI} --output-dir tmp/${id}/secure")
            }
        }
    }
