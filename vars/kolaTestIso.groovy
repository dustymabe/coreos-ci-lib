...
def call(params = [:]) {
    ...
    if (!params['skipUEFI']) {
        testIsoRuns2["metalUEFI"] = {
            ...
            Ids += id
            shwrap("mkdir -p tmp/${id}")
            shwrap("cd ${cosaDir} && kola testiso -S --qemu-firmware=uefi ${extraArgsUEFI} --scenarios ${scenariosUEFI} --output-dir tmp/${id}/insecure")
            shwrap("cd ${cosaDir} && kola testiso -S --qemu-firmware=uefi-secure ${extraArgsUEFI} --scenarios ${scenariosUEFI} --output-dir tmp/${id}/secure")
        }
    }
